CREATE DATABASE exfiltrap;

USE exfiltrap;

CREATE TABLE dns_all_plain (
  plain_id BIGINT PRIMARY KEY COMMENT 'Primary Key. One row per query',
...
CREATE TABLE dns_all_doh (

  doh_id BIGINT PRIMARY KEY COMMENT 'Primary Key. One row per completed flow',
...
CREATE INDEX idx_timestamp ON dns_all_doh(timestamp);
...
CREATE INDEX idx_traffic_type ON dns_all_doh(traffic_type);

CREATE TABLE dns_tunneling_plain (
...
CREATE TABLE dns_tunneling_doh (
...
CREATE TABLE dns_tunnel_events (
...
CREATE TABLE dns_abuse (

  flow_id BIGINT PRIMARY KEY
    COMMENT 'Primary Virtual ID representing the 5-tuple flow',

  doh_id BIGINT NULL
    COMMENT 'Referencing a DoH Flow IF the abuse originated on 443',

  protocol VARCHAR(50),
  protocol_number INT,

  dns_amplification_factor FLOAT,
  query_response_ratio FLOAT,
  dns_any_query_ratio FLOAT,
  dns_txt_query_ratio FLOAT,
  dns_response_inconsistency FLOAT,
  dns_queries_per_second FLOAT,
  port_53_traffic_ratio FLOAT,
  ...
  dns_total_queries FLOAT,
  dns_total_responses FLOAT,
  dns_response_bytes FLOAT,
  ...
  flow_length_min FLOAT,
  flow_length_max FLOAT,

  response_time_variance FLOAT,
  average_packet_size FLOAT,

  large_packet_ratio FLOAT,
  medium_packet_ratio FLOAT,
  small_packet_ratio FLOAT,

  -- HTTPS / DoH Specific Abuse Metrics
  sni_entropy FLOAT,
  is_known_doh_server BOOLEAN,
  encrypted_payload_size_variance FLOAT,

  label VARCHAR(100),

  CONSTRAINT fk_dns_abuse_doh
    FOREIGN KEY (doh_id) REFERENCES dns_all_doh(doh_id)
    ON UPDATE CASCADE
    ON DELETE SET NULL
