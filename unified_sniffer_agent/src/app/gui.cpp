...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam,
  ...
  case WM_CREATE: {
    // Title
    CreateWindow("STATIC", "Select Network Interface:", WS_VISIBLE | WS_CHILD,
                 20, 20, 300, 20, hwnd, NULL, NULL, NULL);

    // Interface Dropdown
    g_hCombo = CreateWindow(
        WC_COMBOBOX, "", CBS_DROPDOWNLIST | WS_CHILD | WS_VISIBLE | WS_VSCROLL,
    ...
    // Prerequisite Note
    CreateWindow("STATIC", "Prerequisites:", WS_VISIBLE | WS_CHILD, 20, 85, 300,
                 20, hwnd, NULL, NULL, NULL);

    g_hStatus = CreateWindow("STATIC", "Checking system status...",
                             WS_VISIBLE | WS_CHILD, 20, 105, 340, 60, hwnd,
                             NULL, NULL, NULL);

    // Notes
    CreateWindow("STATIC",
                 "Note: Ensure Npcap is installed and Database is reachable.",
                 WS_VISIBLE | WS_CHILD | SS_LEFT, 20, 170, 340, 40, hwnd, NULL,
                 NULL, NULL);

    // Start Button
    CreateWindow("BUTTON", "Start Sniffing",
                 WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON, 120, 220, 140, 35,
  ...
  case WM_COMMAND: {
    if (LOWORD(wParam) == 102) { // Start Button
      ...
      if (sel != CB_ERR) {
        pcap_if_t *d =
            (pcap_if_t *)SendMessage(g_hCombo, CB_GETITEMDATA, sel, 0);
        if (d) {
  ...
  case WM_CTLCOLORSTATIC: {
    ...
    if (hwndStatic == g_hStatus) {
      SetBkColor(hdcStatic, GetSysColor(COLOR_WINDOW));
      if (g_db_connected) {
        SetTextColor(hdcStatic, RGB(0, 160, 0)); // Green connection light
      } else {
        SetTextColor(hdcStatic, RGB(220, 0, 0)); // Red connection light
      }
...
GuiResult Gui::show_setup_dialog(pcap_if_t *interfaces, bool db_connected) {
  g_interfaces = interfaces;
  g_db_connected = db_connected;
  g_result = GuiResult();
  ...
  HWND hwnd = CreateWindowEx(
      ...
      WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU, (screenWidth - width) / 2,
      (screenHeight - height) / 2, width, height, NULL, NULL, hInstance, NULL);
  ...
  // Update Status
  std::string status = g_db_connected
                           ? "- Npcap Driver: INSTALLED\r\n- MySQL Server: "
                             "CONNECTED SUCCESSFULLY"
                           : "- Npcap Driver: INSTALLED\r\n- MySQL Server: "
                             "DISCONNECTED (Check IP/Credentials)";
